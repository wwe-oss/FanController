...
{
  if (now - lastRPMCalc < g_config.rpmSampleIntervalMs)
    return;
...
{
  if (now - lastSoftStart < SOFT_START_MS)
    return;
  lastSoftStart = now;
...
{
  if (now - lastSafetyCheck < SAFETY_MS)
    return;
  lastSafetyCheck = now;

  if (duty < MIN_DUTY)
    return; // low or off → ignore
